...
def generate_response(message_body, wa_id=None, name=None):
    ...
    lower_message = message_body.strip()
    step = user_session["step"]

    if step == "ask_name":
        user_session["visitor_info"]["name"] = lower_message
        user_session["step"] = "ask_number"
        session_context[wa_id] = user_session
        return "Please enter the visitor's phone number (with country code, e.g. 234...):"

    elif step == "ask_number":
        if not lower_message.isdigit():
            return "Invalid number format. Please enter digits only, including the country code."
        user_session["visitor_info"]["number"] = lower_message
        ...
        if lower_message == "1" or lower_message == "today":
            selected_date = today.strftime("%Y-%m-%d")
        elif lower_message == "2" or lower_message == "tomorrow":
            selected_date = tomorrow.strftime("%Y-%m-%d")
        elif lower_message.startswith("3") or re.match(r"^\d{4}-\d{2}-\d{2}$", lower_message):
            try:
                # Extract date part in case of "3. 2025-05-26" or just a plain date
                date_str = lower_message.split("3")[-1].strip() if lower_message.startswith("3") else lower_message
        ...
        send_qr_code_to_visitor(visitor_info["number"], qr_image_b64)
        # send_pass_to_visitor(visitor_info["number"], qr_image_b64)
        # send_existing_qr_image_to_visitor("2348034150437", "Ola_20250524_080258.png")
        # send_test_image_to_visitor(visitor_info["number"])
        # send_test_image_to_visitor(2348061698657)
        session_context.pop(wa_id, None)

        return f"Visitor pass created successfully and QR code sent to {visitor_info['number']}."
